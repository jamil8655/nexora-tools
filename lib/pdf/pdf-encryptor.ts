...
function md5(bytes: Uint8Array): Uint8Array {
  ...
  words[nWords - 2] = (length * 8) & 0xffffffff;
  words[nWords - 1] = Math.floor((length * 8) / 0x100000000);
...
/**
 * Genuine PDF Password Protection Engine
 * Protects any PDF with 128-bit standard encryption that prompts for password on all PDF viewers.
 */
